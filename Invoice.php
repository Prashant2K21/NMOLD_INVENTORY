...
class Invoice{
    ...
    private $user  = 'root';
    private $password   = "";
    private $database  = "inventory system";   
	...
	public function saveInvoice($POST) {		
		...
		for ($i = 0; $i < count($POST['productCode']); $i++) {
			$sqlInsertItem = "
			INSERT INTO ".$this->invoiceOrderItemTable."(order_id, item_code, product_name, order_item_quantity, order_item_price, order_item_final_amount) 
	...
	public function updateInvoice($POST) {
		...
		for ($i = 0; $i < count($POST['productCode']); $i++) {			
			$sqlInsertItem = "
				INSERT INTO ".$this->invoiceOrderItemTable."(order_id, item_code, product _name, order_item_quantity, order_item_price, order_item_final_amount) 
	...
	public function getInvoice($invoiceId){
		$sqlQuery = "
			SELECT * FROM ".$this->invoiceOrderTable." 
			WHERE user_id = '".$_SESSION['user']['id']."' AND order_id = '$invoiceId'";
		$result = mysqli_query($this->dbConnect, $sqlQuery);	
